...
YESTERDAY_LOG="cron_etl_assign_40_delivered_$(date -d "yesterday" +'%Y%m%d').log"
...
LOG_FILE="cron_etl_assign_40_delivered_$(date +'%Y%m%d').log"
...
poetry run etl_assign_40_delivered >> /opt/delivery-tracking-etl/$LOG_FILE 2>&1